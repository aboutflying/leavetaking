...
def test_filter_corporate_pacs_includes_connected_org_name():
    ...
    committees = [{'connected_org_name': 'ACME Corp', 'designation': 'U', 'interest_group_category': ''}]
...
def test_filter_supported_contributions_handles_whitespace_transaction_type():
    ...
    result = list(filter_supported_contributions(rows))
    assert len(result) == 1
    assert result[0]['transaction_type'] == ' 24K '  # original value preserved, only stripped for comparison
